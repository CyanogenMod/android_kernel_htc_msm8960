...
int SPI_Read32( uint16_t uwIndex , uint16_t uwCount , uint32_t * pData)
{
	...
	for (i=0; i<uwCount; i++)
	{
		...
		rc = rawchip_spi_read_2B4B(reg,&val);
		if (rc==0)
		{
			*(pData+i)=val;
...
int rawchip_spi_read_2B4B(uint16_t addr, uint32_t *data)
{
	...
	rc = spi_write_and_read(rawchip_dev, tx_buf, rx_buf, 4);
	if (rc < 0) {
	...
	*data = rx_buf[1];
	CDBG("[CAM]rawchip_spi_read_2B1B---, rc=%d, (addr=0x%x, data=0x%x)\n", rc, addr, *data);
...
#if 0
int rawchip_spi_read_big_buffer(uint16_t addr, unsigned char *data, int count)
{
	unsigned char buffer[4], tx_buf[2];
	unsigned char rx_buf[MAX_SIZE+1];
	int rc = 0;
	int times, i;
	int data_acc=0, times_acc=0,left=0;
	int read_size;
	times = (count /MAX_SIZE) + 1;
	rawchip_dev->bits_per_word = 8;

	for(i=0 ; i<times ; i++){
		
		buffer[0] = 0x60;
		buffer[1] = ((addr+times_acc) & 0xff00) >> 8;
		buffer[2] = (addr+times_acc) & 0x00ff;
		...
		tx_buf[0] = 0x61;
		tx_buf[1] = 0x00;
		rc = spi_write_and_read(rawchip_dev, tx_buf, rx_buf, read_size+1);
		if (rc < 0) {
			pr_err("[CAM]rawchip_spi_read_burst spi_write_and_read failed, rc=%d count:%d\n", rc, count);
			return rc;
		}
		memcpy(data+data_acc, rx_buf+1, read_size);
		data_acc = data_acc + read_size;
		times_acc = times_acc+MAX_SIZE;
	}
	CDBG("[CAM]rawchip_spi_read_burst---, \
		rc=%d, (addr=0x%x, data=0x%x rx_buf[0]:0x%x)\n", rc, addr, data[0], rx_buf[0]);
	return rc;
