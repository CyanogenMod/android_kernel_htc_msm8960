...
void RamRead32A_lc898111(uint16_t addr, unsigned long* data)
{
	int32_t rc = 0;
	uint8_t *data_ptr;
	uint8_t dummy[4];
	...
	rc = msm_camera_i2c_read_seq(binder_i2c_client, addr, (uint8_t *)data, 4);
	if (rc < 0) {
		pr_err("%s i2c read failed (%d)\n", __func__, rc);
		return;
	}
...
void HtcActOisBinder_i2c_add_driver(struct msm_camera_i2c_client* cam_i2c_client)
{
	...
	if (binder_i2c_client == NULL) {
		...
		memcpy(binder_i2c_client, cam_i2c_client, sizeof(struct msm_camera_i2c_client));
		...
		pr_info("[OIS]  OLD binder_i2c_client->client->addr=0x%x\n", binder_i2c_client->client->addr);
		pr_info("[OIS]  OLD cam_i2c_client->client->addr=0x%x\n", cam_i2c_client->client->addr);
		binder_i2c_client->addr_type = MSM_CAMERA_I2C_WORD_ADDR;
		binder_i2c_client->client->addr = OIS_I2C_ADDR;
		pr_info("[OIS]  NEW binder_i2c_client->client->addr=0x%x\n", binder_i2c_client->client->addr);
		pr_info("[OIS]  NEW cam_i2c_client->client->addr=0x%x\n", cam_i2c_client->client->addr);
	}
