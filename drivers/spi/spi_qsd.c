...
static irqreturn_t msm_spi_qup_irq(int irq, void *dev_id)
{
	...
	if (!dd->update_complete)
		pr_err("[SPI] SPI_ERR: %s Should not get irq now\n", __func__);
...
static void msm_spi_read_word_from_fifo(struct msm_spi *dd)
{
	...
	if (dd->read_buf) {
		...
		if (dd->read_buf == NULL)
			pr_info("[SPI] SPI_ERR: BF data_in = %x\n", data_in);
		else
		...
		for (i = 0; (i < dd->bytes_per_word) &&
			...
			*/
			...
			if (dd->read_buf == NULL) {
				...
				pr_info("[SPI] SPI_ERR: i=%d,rem=%d,cur=%d,bpw=%d,data_in=%x,owbuf[0]=%x,owbuf[1]=%x,owbuf[2]=%x,owbuf[3]=%x,nwbuf[0]=%x,nwbuf[1]=%x,nwbuf[2]=%x,nwbuf[3]=%x,tmp_read_buf_add=%p update_complete=%d\n",
