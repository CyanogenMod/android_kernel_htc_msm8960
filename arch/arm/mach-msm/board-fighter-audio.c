...
#define TPA2051_PAMP_GPIO	(PM8921_GPIO_PM_TO_SYS(18))
#define HAC_PAMP_GPIO	(PM8921_GPIO_PM_TO_SYS(19))
...
static int fighter_spk_power_amp_on(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		msm8960_ext_spk_power_amp_on(
			BOTTOM_SPK_AMP_POS|BOTTOM_SPK_AMP_NEG);
		break;
	case SND_SOC_DAPM_POST_PMD:
		msm8960_ext_spk_power_amp_off(
			BOTTOM_SPK_AMP_POS|BOTTOM_SPK_AMP_NEG);
		break;
	}
...
int fighter_get_component_info(void)
{
	uint32_t audio_hw_comp = 0;

	if (system_rev < 3)
	    audio_hw_comp = HTC_AUDIO_TPA2051;
...
static struct acoustic_ops acoustic = {
	...
	.get_hw_component = fighter_get_component_info,
};
